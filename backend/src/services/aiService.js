...
exports.extractEntitiesFromEmail = extractEntitiesFromEmail;
    ...
    properties: {
        tasks: {
            ...
            items: {
                type: genai_1.Type.OBJECT,
                properties: {
                    title: { type: genai_1.Type.STRING, description: "A short, concise title for the task." },
                    description: { type: genai_1.Type.STRING, description: "Additional details, links, or context." },
                    dueDate: { type: genai_1.Type.STRING, description: "Deadline in ISO 8601 format, or null." },
                    priority: { type: genai_1.Type.STRING, description: "'High', 'Medium', or 'Low'." },
                    category: { type: genai_1.Type.STRING, description: "'Education', 'Work', 'Finance', 'Personal', or 'Other'." }
                },
                required: ["title"]
            }
        },
        events: {
            ...
            items: {
                type: genai_1.Type.OBJECT,
                properties: {
                    title: { type: genai_1.Type.STRING, description: "Title of the event or meeting." },
                    eventDate: { type: genai_1.Type.STRING, description: "Date and time of the event in ISO 8601 format." },
                    eventTime: { type: genai_1.Type.STRING, description: "Human readable time string, e.g. '3:00 PM'." },
                    location: { type: genai_1.Type.STRING, description: "Location or virtual meeting link." }
                },
        ...
        bills: {
            ...
            items: {
                type: genai_1.Type.OBJECT,
                properties: {
                    company: { type: genai_1.Type.STRING, description: "Company or service name (e.g. 'Netflix', 'Electricity')." },
                    amount: { type: genai_1.Type.NUMBER, description: "The amount due. Provide a number only." },
                    currency: { type: genai_1.Type.STRING, description: "Currency symbol or code (e.g. '$', 'USD', 'EUR')." },
                    dueDate: { type: genai_1.Type.STRING, description: "The deadline for the bill in ISO 8601 format." },
                    type: { type: genai_1.Type.STRING, description: "'Utility', 'Subscription', 'Credit Card', or 'Other'." }
                },
                required: ["company"]
            }
        },
...
async function extractEntitiesFromEmail(subject, sender, body, date) {
    try {
        const prompt = `
